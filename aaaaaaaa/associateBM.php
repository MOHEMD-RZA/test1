...
<head>
	                <meta charset="utf-8"> 
	                <title>مقرر قسم أدارة الاعمال</title> 
                  <link rel="stylesheet" href="aassoo.css"> 	       
</head>  
<body dir="rtl">
          <form action="associateBM.php" method="POST">
      ...
      <div class="m" style="width: 98.5%; height: 79%; border: 2px  solid #000"><!--لانشاء المستطيل-->
           ...
           <select class="te1" name='stage'>
            ...
            <input type="text" value="تقنيات أدارة الاعمال" class="te3" name='namee'>
                    ...
                    ?>
                          <!-- عرض البيانات -->
                          <?php
                    $result=mysqli_query($conn,"select *from student where sec_name='تقنيات أدارة الاعمال' AND zo=1 AND stage !='خريج'");
                    ...
                    while($row=mysqli_fetch_array($result)){
                      ...
                      echo "<td class='hh'><a  target=_blank href='bbbBM.php?id=$row[id]' >تعديل</a></td>"; //target=_blankلفتح صفحة اخرى 
                      echo "<td  class='hh'><a href='deBM.php?id=$row[id]' >حذف</a></td>";
                        echo "</tr>";
                    }
                    echo "</table>";?>
                     </div>
